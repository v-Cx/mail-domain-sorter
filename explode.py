    ...
    if(isinstance(word, list)):
        ...
        for words in word:
            ...
            for line in open(file_path.name, 'r').readlines():
                ...
                if words in line:
                    ...
                    """
                    i += 1
                    open('result/' + os.path.basename(str(words) + '.txt'), 'w+')
            ...
            if control == True:
                ...
                """
                open("result/" + str(words) + '.txt', "w").writelines(findList)
            ...
            time.sleep(0.5)
            print("---------------------------------------------------")
            print(str(len(open('result/' + words +  '.txt').read().splitlines())) + " satır " + str(words) + " bulundu")
            print("---------------------------------------------------")
            time.sleep(0.5)
        ...
        for line in open(file_path.name, 'r').readlines():
            if word in line:
                control = True
                i += 1
                open('result/' + os.path.basename(str(word) + '.txt'), 'w+')
                findList.append(line)
        if control == True:
            open("result/" + str(word) + '.txt', "w").writelines(findList)
        else:
            print("No line found about " + word)


        print("---------------------------------------------------")
        print(str(len(open('result/' + word +  '.txt').read().splitlines())) + " satır " + word + " bulundu")
        print("---------------------------------------------------")
